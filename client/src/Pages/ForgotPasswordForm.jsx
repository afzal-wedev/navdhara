...
const ForgotPasswordForm = () => {
  ...
  const handleRequestOtp = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${import.meta.env.VITE_API_URL}/auth/forgot/request`, { email });
  ...
  const handleResetPassword = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${import.meta.env.VITE_API_URL}/auth/forgot/reset`, { email, otp, newPassword });
      alert(res.data.message);
      setStep(1);
